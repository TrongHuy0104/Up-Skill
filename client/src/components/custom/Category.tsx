        ...
        >
          ...
          {categories.map((category: Category) => (
            <li
              ...
              className="relative flex justify-between items-center px-2 cursor-pointer text-base font-medium hover:bg-accent-100 
            ...
            >
              <Link href={`/courses?category=${category.title}`} className="w-full h-full flex items-center px-3 py-[15px]">
                {category.title}
              </Link>
              ...
              {hoveredCategory === category._id && category.subCategories && category.subCategories.length > 0 && (
                ...
                >
                  {category.subCategories.map((sub) => (
                    <li key={sub._id} className="relative flex justify-between items-center text-primary-800 text-base font-medium hover:bg-accent-100 hover:text-accent-900 cursor-pointer">
                      <Link href={`/courses?category=${category.title}&subCategory=${sub.title}`} className="w-full flex items-center px-3 py-[15px]">
                        {sub.title}
                      </Link>
                    </li>
                  ))}
